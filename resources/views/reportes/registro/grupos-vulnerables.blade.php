...
@section('content')
                    ...
                    <div class="report-road-fields-grid grid grid-cols-1 gap-x-3 gap-y-3 lg:grid-cols-2 lg:gap-x-4">
                        ...
                        <div>
                            ...
                            @else
                                ...
                                <input id="jurisdiction_display_gv" type="text" class="ui-field--disabled" value="{{ optional(auth()->user()->district)->display_name }}" disabled aria-disabled="true">
                            @endif
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            const municipalityToDistrict = @json($municipalities->mapWithKeys(fn($item) => [$item->id => $item->district_id])); const districtNames = @json($districts->mapWithKeys(fn($item) => [$item->id => $item->display_name]));
