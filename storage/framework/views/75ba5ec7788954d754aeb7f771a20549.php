<div class="bg-header text-white p-2 lg:p-3 w-full h-14 lg:h-16 font-sans flex items-center justify-start gap-4 lg:gap-6">
    ...
    <a href="<?php echo e(auth()->check() ? (auth()->user()->hasAnyRole(['Administrador','Coordinador']) ? route('statistic.data') : route('reportes.index')) : route('login')); ?>" class="flex items-center flex-shrink-0">
    ...
    <div class="hidden md:flex items-center gap-8 lg:gap-12">
        <a href="<?php echo e(route('reportes.index')); ?>" class="text-base lg:text-base font-semibold hover:underline transition-all whitespace-nowrap">Reportes</a>
        <a href="<?php echo e(route('statistic.data')); ?>" class="text-base lg:text-base font-semibold hover:underline transition-all whitespace-nowrap">Estadísticas</a>
    </div>
    ...
    <div class="flex items-center gap-3 lg:gap-4 flex-shrink-0">
        ...
        <div class="relative flex items-center ml-3 lg:ml-4" x-data="{ openNotifications: false, notifications: [], unreadCount: 0 }" 
            ...
            <div x-show="openNotifications" 
                ...
                <div class="px-3 lg:px-4 py-2 lg:py-3 border-b border-gray-200 bg-gray-50 flex justify-between items-center">
                    ...
                    <button @click="
                        fetch('/notificaciones/marcar-todas-leidas', { method: 'POST', headers: { 'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>' } })
                            .then(() => {
        ...
        <div class="relative flex items-center gap-3 lg:gap-4" x-data="{ openProfile: false }">
            ...
            <a href="<?php echo e(route('usuario.miperfil')); ?>" class="flex items-center gap-2.5 lg:gap-3.5 flex-shrink-0">
                ...
                <div class="text-left hidden lg:block min-w-0 w-64 h-9 overflow-hidden">
                    <p class="text-base font-semibold truncate leading-tight"><?php echo e(trim(implode(' ', array_filter([auth()->user()->name, auth()->user()->first_last_name])))); ?></p>
                    <p class="text-sm text-gray-300 truncate leading-tight"><?php echo e(auth()->user()->position->name ?? 'Sin cargo'); ?></p>
                </div>
            </a>
            ...
            <div x-show="openProfile" 
                ...
                <div class="px-3 lg:px-4 py-2 lg:py-3 border-b border-gray-100 bg-gray-50">
                    ...
                    <p class="text-xs text-gray-500 mt-1"><?php echo e(trim(implode(' ', array_filter([auth()->user()->name, auth()->user()->first_last_name, auth()->user()->second_last_name]))) ?? 'Usuario'); ?></p>
                </div>
...
</div><?php /**PATH C:\Proyectos Laravel\sistema-sec-tam\resources\views\components\header.blade.php ENDPATH**/ ?>