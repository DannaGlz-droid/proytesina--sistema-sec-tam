...
unset($__errorArgs, $__bag); ?>
                ...
                <input type="hidden" id="jurisdiction_input" name="district_id" value="<?php echo e(old('district_id', $defuncion->district_id ?? '')); ?>">
                <input type="text" id="jurisdiction" 
                    ...
                    readonly
                    value="<?php echo e(old('district_id') ? ($jurisdictions->firstWhere('id', old('district_id'))->name ?? '') : (optional($defuncion->jurisdiction)->name ?? '')); ?>">
                        </div>
                        <div>
...
<?php endif; ?>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const muniToJur = <?php if(isset($municipalities)): ?> <?php echo json_encode($municipalities->mapWithKeys(function($m){ return [$m->id => $m->district_id, mb_strtolower($m->name) => $m->district_id]; }), 512) ?> <?php else: ?> {} <?php endif; ?>;
