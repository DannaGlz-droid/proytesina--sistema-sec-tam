...
- Los detalles contextuales que amplían una métrica sin cambiar su universo también pertenecen a Presentación. En Edades, ofrecer una elección explícita entre “Solo edades” y “Edades y causas”; la segunda conserva la gráfica y añade debajo un resumen integrado con cada grupo, su total y hasta tres causas ordenadas. No esconder esta elección en Filtros ni presentar el detalle como una tabla administrativa: usar filas jerárquicas, superficie neutral, tipografía y separadores del panel estadístico, valores tabulares y adaptación a una sola columna en móvil.
...
- Las barras horizontales conservan una fila legible por categoría: al mostrar listados extensos, el lienzo crece con la página en lugar de comprimir todas las filas dentro de una altura máxima. Como referencia, reservar alrededor de 34 px por categoría y evitar barras menores de 20–22 px; Top 5, Top 10 y Top 15 pueden usar barras progresivamente más compactas sin reducir la tipografía antes de Top 15. Las etiquetas de valor y porcentaje permanecen fuera del extremo de la barra con separación suficiente y sin solaparse.
- Las gráficas de pastel y rosquilla priorizan comparaciones de pocas categorías. En escritorio, Top 5 sustituye la leyenda por llamadas exteriores: una línea guía discreta conecta cada sector con el nombre de la categoría y el valor, porcentaje o ambos elegido en Presentación. No usar puntas de flecha ni duplicar esa información en una leyenda. En pantallas estrechas o cuando los nombres sean extensos, conservar la leyenda por seguridad. A partir de Top 10, los valores se integran en la leyenda para evitar líneas y textos superpuestos. Cuando la selección supera 15 categorías, representar como máximo los diez segmentos principales y agrupar el resto en un único segmento neutral identificado como “Resto (n)”. La leyenda no debe paginarse en escritorio si esta agrupación permite mostrarla completa. En una misma vista de escritorio, el diámetro exterior y el grosor del anillo permanecen estables al alternar entre Top 5, Top 10, Top 15 y Todos; los cambios de altura del panel no deben escalar el círculo. Solo se reduce de forma responsiva cuando el ancho o alto disponible realmente lo exige.
...
- La tarjeta incluye un pie compacto de trazabilidad con el origen de los registros filtrados. El resumen prioriza el número de importaciones y menciona la captura manual únicamente cuando aporta registros. “Ver detalle” muestra el nombre original y la cantidad aportada por cada carga, además de los registros manuales; nunca presenta el archivo como descargable si el sistema solo conserva sus metadatos.
