...
- Las etiquetas procedentes de catálogos se muestran con capitalización normal y la misma redacción en gráficas, filtros, tablas, formularios y exportaciones, aunque su valor canónico permanezca en mayúsculas. Conservar en mayúsculas únicamente siglas y claves oficiales (`IMSS`, `ISSSTE`, `DIF`, números romanos); por ejemplo, mostrar `Otro`, `Otros accidentes` e `IV · Reynosa`, no `OTRO`, `OTROS ACCIDENTES` ni `IV - REYNOSA`. Esta normalización pertenece a un formateador compartido de presentación y no se resuelve con `text-transform` ni modificando el dato almacenado.
...
- En escritorio amplio, los controles de presentación forman una columna compacta de `15rem` a `17rem` a la izquierda y la visualización ocupa el resto. La columna se alinea arriba, conserva altura automática y nunca se estira para igualar la gráfica cuando una métrica ofrece menos opciones.
- Por debajo de `1280px`, Presentación vuelve encima de la gráfica: encabezado lateral y grupos fluidos en tablet horizontal, dos columnas en tablet estrecha y una columna en móvil. La gráfica nunca debe quedar por debajo de un ancho útil razonable solo por conservar la disposición lateral.
...
- Los filtros se ofrecen según la dimensión analizada, no como una lista idéntica para todas las pestañas. Municipios no se filtra por municipio; Edades no se filtra por edad; Sexo no se filtra por sexo; Causas no se filtra por causa; Distritos no se filtra por distrito ni municipio. Tendencias puede cruzarse con todos esos criterios. En Comparativa, ocultar únicamente el filtro que duplicaría una de sus dimensiones: sexo en Sexo por causa, edad en Edad por causa y causa en las comparativas por causa; las comparaciones entre residencia y defunción no admiten un ámbito geográfico único porque representan ambos simultáneamente.
- El tipo de comparativa no es un filtro ni una opción de presentación. Mostrarlo como selector contextual `Comparar` dentro del encabezado de la gráfica, antes del periodo y visible únicamente en Comparativa. Su cambio se aplica de inmediato; no debe depender del botón “Aplicar” ni incorporar cambios pendientes del panel de filtros. Usar nombres explícitos para las dimensiones, por ejemplo `Municipio de residencia vs. municipio de defunción`.
- “Ámbito geográfico” define de forma conjunta las listas y consultas de municipio y distrito: Lugar de defunción usa municipio y distrito de defunción; Residencia usa municipio y distrito de residencia. La misma elección debe llegar a la gráfica, Ver datos, CSV y demás exportaciones. Las etiquetas visibles y los chips explicitan el ámbito para evitar mezclar ambos conceptos.
- “Últimos 12 meses (predeterminado)” representa la ventana automática inicial; “Todas las fechas” es una elección explícita distinta y no debe activar esa ventana. En el contexto de la gráfica se muestra siempre la forma neutral `Periodo: fecha inicial – fecha final`, tanto para la ventana automática como para un rango filtrado, porque las fechas comunican el alcance sin exponer una condición interna del sistema. No calcular ni mostrar una variación automática frente al periodo anterior: los filtros de fecha definen únicamente el universo actual y una comparación temporal futura deberá permitir elegir explícitamente ambos periodos. Los años aceptan un año, listas o intervalos; los meses no seleccionados dentro de un intervalo no se incluyen implícitamente. Los periodos personalizados, trimestres y edades se validan antes de aplicar y muestran el error junto al control correspondiente.
...
- “Limpiar” modifica el borrador del panel; el gráfico cambia únicamente después de aplicar. Los filtros efectivamente aplicados reutilizan el mismo componente de chip granate de los listados y aparecen a continuación del botón Filtros dentro de la misma toolbar. Cada chip identifica primero el criterio y permite retirarlo individualmente; si el ancho no alcanza, los chips envuelven a la línea siguiente y en móvil ocupan una fila propia. El grupo completo desaparece cuando no hay filtros aplicados.
- Tipo de gráfica, etiquetas de datos, cantidad visible y paleta de colores son controles de presentación, no filtros de datos. Permanecen en un panel visible independiente y pueden actualizar la visualización de inmediato porque no alteran el universo filtrado. Si una métrica solo admite un tipo de gráfica, ocultar ese grupo en vez de mostrar una única opción sin capacidad de elección.
- Los detalles contextuales que amplían una métrica sin cambiar su universo también pertenecen a Presentación. En Edades, agrupar el control bajo “Desglose adicional” y usar el checkbox “Mostrar principales causas por edad”, con la ayuda “Muestra las tres causas con más registros en cada grupo de edad.” El detalle añade un resumen rápido a la gráfica existente y no sustituye la comparación completa de rango etario por causa. Este control reutiliza el checkbox neutral del sistema; su hover únicamente refuerza el borde de la casilla y no cambia el texto a color institucional. Al marcarlo, la casilla confirma el cambio con una escala breve y el resumen entra mediante opacidad y un desplazamiento vertical mínimo; al desmarcarlo, la salida es más corta. Este movimiento solo acompaña cambios de visibilidad, no se repite al actualizar datos, y se elimina con `prefers-reduced-motion`. Al marcarlo, conservar la gráfica y añadir debajo un resumen integrado con cada grupo, su total y hasta tres causas ordenadas. No esconder esta opción en Filtros ni presentar el detalle como una tabla administrativa: usar filas jerárquicas compactas, superficie neutral, tipografía y separadores del panel estadístico y valores tabulares. El resumen es información secundaria: alinear su ancho con el área útil de trazado, distribuir las causas horizontalmente cuando haya espacio y apilarlas solo en móvil; nunca debe ocupar más ancho ni competir en altura o jerarquía con la gráfica. En escritorio, identificar el orden una sola vez mediante cabeceras alineadas “1.ª causa”, “2.ª causa” y “3.ª causa”; las cabeceras usan una superficie neutral diferenciada, contraste suficiente y peso semibold o bold sin superar la jerarquía del título del detalle. No repetir insignias en cada fila ni dejar números sueltos que puedan confundirse con cantidades. Al apilar en móvil, repetir la posición como texto breve dentro de cada causa. La exportación conserva el ancho, la altura del lienzo y el ritmo vertical de la vista. Los controles y el pie se ocultan sin retirarlos del flujo durante la rasterización; cualquier área que no deba aparecer se recorta después sobre el bitmap, evitando que el encabezado del detalle cambie de posición respecto de la gráfica.
...
- El selector de paleta es un control compartido por Municipios, Tendencias, Edades, Sexo, Causas, Distritos y Comparativa, pero sus alternativas y muestras son contextuales. Pastel y Dona ofrecen únicamente paletas cualitativas con matices distinguibles (`Granate, oro y azul`, `Azul, ámbar y verde` y `Azul, tierra y verde`); mapas y mapas de calor excluyen paletas divergentes y muestran una banda secuencial; Comparativa muestra los colores asignados a sus series. Los nombres describen los tonos visibles en vez de usar términos genéricos como “Multicolor” o “Natural”; las opciones de tinta única indican el tono concreto, por ejemplo `Granate profundo`, `Azul petróleo` o `Café tierra`. Cuando un contexto ofrece ambos tipos, el menú agrupa primero las alternativas en `Paletas de varios colores` y después en `Colores sólidos`, con un separador neutral; no mezcla ambos tipos por prioridad ni los oculta bajo “Explorar más paletas”. Los mapas y mapas de calor usan el encabezado específico `Escalas de color`. Todas las alternativas usan una banda de muestra con las mismas dimensiones y posición: cuando Columnas, Barras, Línea o Área utilizan una sola tinta, la banda se rellena por completo con ese color; cuando una paleta contiene varios matices, la misma banda se divide en segmentos. El control cerrado repite este patrón para conservar la alineación y evitar que colores y paletas parezcan controles distintos. Las distribuciones de dos a cinco categorías que usan color cualitativo muestran exactamente esos colores en la banda. Su popover puede ser más ancho que el panel de Presentación para conservar nombres en una línea y organizar las alternativas en dos columnas compactas; en escritorio debe disponer de altura suficiente para mostrar completas las opciones antes de recurrir al desplazamiento interno. Cada alternativa incluye su nombre completo y una marca discreta de selección. La vista previa se recalcula al cargar datos y al cambiar de métrica o familia visual para corresponder al resultado real.
- En el selector de color, una lista corta (hasta tres alternativas) se presenta como filas de ancho contenido, sin huecos de cuadrícula. Las listas más largas conservan dos columnas de opciones compactas. El popover mide su contenido y solo activa desplazamiento interno cuando la lista supera la altura útil del menú o del viewport; el control cerrado no cambia de tamaño al alternar tipos de gráfica. Cuando haya desplazamiento, la barra nativa debe compartir grosor, tono neutral y estado hover con la de Filtros; no introducir un estilo aislado. El desplazamiento debe permitir llegar a la última opción completa y el menú no debe recalcular su altura al desplazarse dentro de él.
- Descargar o exportar es una acción secundaria neutral. El botón primario institucional se reserva para “Aplicar filtros” dentro del popover. La descarga de gráficas usa un botón dividido: la acción principal genera el PNG con fondo blanco y la flecha abre formatos adicionales. Al abrirse, la flecha gira y su nombre accesible cambia de “Abrir” a “Cerrar opciones de descarga”; el panel entra con la misma transición breve de los popovers y se cierra con Escape o clic exterior. “Incluir contexto” emplea el checkbox neutral compartido del sistema y especifica que solo modifica PNG y PDF. Las opciones conservan semántica nativa de botón y checkbox; no declarar `role="menu"` sin implementar también su navegación de teclado especializada.
- El grupo de cantidad usa la leyenda breve “Mostrar” y se adapta a la familia visual. Conserva el indicador `∞` con la etiqueta compacta “Todos”; la cantidad del universo se comunica en el texto de apoyo y en el nombre accesible, no dentro del botón. Cuando se aplica un Top, el encabezado evita el tecnicismo “cobertura” y usa la forma explícita `Mostrando 10 de 44 municipios · representan 79.9% del total`; este contexto también acompaña la exportación. Se oculta al mostrar todo el universo. Solo ofrece los límites `5`, `10` o `15` que oculten al menos tres categorías del universo disponible; un recorte menor no justifica perder contexto. Si el Top preferido deja de ser útil al aplicar filtros, mostrar todas antes que descender automáticamente a un Top mucho menor. Distritos inicia en “Todos” tanto en Columnas como en Barras y solo ofrece “Top 5” como resumen opcional; no ofrece “Top 10” aunque pueda ocultar tres categorías al incluir “Otro”, porque el recorte apenas simplifica esa vista. Su elección de cantidad se recuerda por orientación, sin heredar el Top elegido en otras métricas. Para evitar repetir cada número, las opciones numéricas muestran únicamente “Top”, aunque su nombre accesible permanece completo. Las etiquetas no se dividen en dos líneas; si no caben, los grupos de presentación se reorganizan antes de comprimir o cortar el texto.
- Los grupos de opciones del panel de Presentación usan preferentemente una cuadrícula de dos columnas y conservan el mismo ancho por opción. Si el grupo tiene un número impar de alternativas, la última permanece en su columna y se alinea a la izquierda; no se estira para completar la fila porque eso le otorgaría una jerarquía distinta. Solo una opción realmente única puede ocupar ambas columnas; si no representa una decisión para la persona usuaria, considerar ocultar el grupo en lugar de mostrar un control redundante.
- Las etiquetas categóricas de una gráfica se adaptan al ancho y al número de categorías mediante envoltura completa por palabras y una reserva uniforme bajo el eje. No abreviar ni truncar con puntos suspensivos nombres que identifican categorías estadísticas. El área de la gráfica crece cuando se requieren más líneas. En barras horizontales, envolver también el nombre completo y aumentar la altura según el número de filas.
- Las columnas muestran como máximo 15 categorías cuando el universo es mayor. Municipios inicia en Top 10 para priorizar la comparación y mantiene “Todos” como consulta opcional del universo con registros. En Columnas y Barras de Municipios, cada categoría usa por defecto la misma tinta principal; solo una elección explícita de “Por categoría” diferencia las vistas de dos a cinco categorías. El orden descendente y la longitud ya comunican jerarquía, por lo que no se destaca automáticamente un subconjunto mediante otro color ni se añade una nota explicativa. Las categorías se ordenan por cantidad descendente y alfabéticamente cuando hay empate. Las columnas se compactan dentro del ancho disponible y conservan un grosor mínimo. Cuando Municipios muestra más de 15 categorías, tanto los nombres como sus valores se giran 90 grados y conservan la misma dirección de lectura. Los valores se centran ópticamente sobre cada columna, crecen hacia arriba desde su extremo y mantienen una separación clara; los nombres usan al menos 11 px y un margen suficiente respecto del eje. En el modo “Valor y %”, todas las vistas de Columnas y Barras de Municipios usan una sola etiqueta con el formato `21.2% (169)`, priorizando el porcentaje y evitando dos líneas paralelas. El formato permanece horizontal en los Top de Columnas y en todas las Barras; gira únicamente en la vista completa de Columnas. Top 5 y Top 10 mantienen cada nombre en una sola línea. Top 15 también usa una línea y tipografía de 11 px cuando cada categoría dispone de al menos 70 px; solo recurre a saltos completos por palabras en anchos menores para impedir solapamientos. El lienzo reserva espacio adicional para que ninguna etiqueta se recorte. No introducir desplazamiento horizontal ni generar una descarga panorámica. Top 5, Top 10 y Top 15 permanecen disponibles para una lectura resumida. Las demás métricas con alta densidad ofrecen la acción contextual “Ver todas en Barras”. Las barras horizontales pueden representar su universo completo porque conservan una fila legible por categoría y hacen crecer el lienzo con la página; el control “Todos” ya comunica este alcance y no debe repetirse mediante una nota auxiliar. En “Todos” de Municipios se usa una densidad cercana a 27 px por fila y barras de 18 px; los Top mantienen alrededor de 34 px por fila y un grosor progresivamente mayor. Las etiquetas permanecen fuera del extremo de la barra con separación suficiente y sin solaparse.
- En Municipios, cambiar de Columnas a Barras conserva el Top activo de Columnas mientras Barras no tenga una cantidad elegida explícitamente; el cambio de orientación no amplía por sí mismo de Top 10 a Todos. Si el usuario elige Todos u otro Top en Barras, esa preferencia se recuerda al alternar de nuevo. Restablecer Presentación devuelve Columnas a Top 10 y elimina la preferencia propia de Barras.
- Edades y Sexo admiten tanto columnas como barras horizontales aunque tengan pocas categorías. Columnas permanece como vista predeterminada; Barras ofrece una alternativa de comparación directa y conserva los nombres completos en el eje vertical.
- Municipios, Edades, Sexo, Causas y Distritos comparten las mismas reglas de presentación categórica en Columnas y Barras: una sola tinta por defecto y varios colores solo cuando el usuario elige una paleta cualitativa para dos a cinco categorías, valores fuera de las barras, formato compacto `porcentaje (valor)` cuando se solicitan ambos, nombres completos, densidad adaptativa y altura creciente en Barras. Estas reglas también se aplican a cualquier distribución categórica simple incorporada posteriormente. Comparativa conserva un color por serie porque en esa vista el color distingue variables diferentes, y Tendencias usa la tinta principal en su línea o área.
- En Tendencias, si todos los puntos corresponden a meses de un mismo año, el eje horizontal muestra únicamente `Ene`, `Feb`, `Mar`… en posición horizontal; el periodo superior conserva el contexto anual. Para series con varios años u otra granularidad se mantienen las etiquetas completas y se giran solo cuando ayuda a evitar solapamientos. Área utiliza un relleno degradado ligero que mantiene la línea como elemento principal.
- Al cambiar de métrica, el sistema adapta automáticamente el tipo de gráfica a una opción compatible sin mostrar mensajes técnicos como “Esta vista usa Columnas; se conserva Área”. El estado activo de los controles comunica por sí mismo qué presentación está aplicada.
- En Barras, los conjuntos de dos a cinco categorías no distribuyen las filas por toda la altura del panel. El bloque se centra verticalmente con separación progresiva y un grosor moderado; así Sexo y cualquier resultado filtrado de baja cardinalidad conservan una composición compacta equivalente a Columnas, mientras los listados extensos mantienen su crecimiento por fila.
- En Columnas de Causas, el eje usa alias compactos de máximo dos líneas y abrevia el sufijo “residencia” como “(res.)” para conservar el significado sin sobrecargar la base de la gráfica; por ejemplo, “Vehículo motor (res.)” y “Expo. a fuego y humo (res.)”. Esta abreviación es únicamente visual: el tooltip, “Ver datos” y las exportaciones tabulares mantienen el nombre completo del catálogo.
- En Barras de Causas se prefieren los nombres completos en una sola línea cuando el ancho de escritorio lo permite; el margen izquierdo puede crecer porque esta orientación está destinada a categorías extensas. En anchos menores, las etiquetas vuelven a envolver por palabras.
- En Columnas de Distritos, “Todos” conserva los nombres completos de las categorías disponibles con tipografía de 11 px y permite hasta dos líneas para evitar que la gráfica se perciba reducida; no se gira el texto. En “Top 5”, cada identificador y municipio permanecen en una sola línea cuando caben en el ancho real. En pantallas estrechas, las etiquetas se envuelven por palabras para evitar solapamientos.
- Al cambiar el tipo de gráfica, Columnas, Barras, Pastel y Dona esperan a que el lienzo tenga dimensiones estables antes de iniciar una animación de entrada consistente. Si la nueva visualización reutiliza el mismo alcance, dibujarla de inmediato. Si requiere consultar un alcance diferente, conservar la gráfica anterior hasta recibir el conjunto correcto y entonces sustituirla una sola vez mediante la animación normal; no dibujar un estado intermedio con categorías que no corresponden a la selección ni reemplazar el lienzo por el estado principal “Cargando datos…”. Los cambios de métrica reutilizan una caché asociada a la métrica, los filtros y el límite visible; después de la primera carga se precargan en segundo plano las demás pestañas, al enfocar o apuntar una pestaña se adelanta su precarga y la caché se invalida al aplicar, retirar o limpiar filtros. Si la respuesta de un Top ya contiene todas las categorías disponibles, reutilizarla al adaptar el control a “Todos” en vez de repetir la consulta. Cuando una métrica todavía no está lista, conservar la gráfica anterior y sustituirla directamente al recibir la nueva; no mostrar el skeleton entre métricas. El skeleton se reserva para la carga inicial sin ninguna gráfica disponible y se retrasa al menos 150 ms para evitar parpadeos en respuestas rápidas. No aplicar las opciones de una gráfica cartesiana en el mismo ciclo de creación del lienzo, porque el navegador puede mostrar directamente su estado final. Respetar `prefers-reduced-motion` y omitir la animación cuando esté activo.
- Las gráficas de pastel y dona priorizan comparaciones de pocas categorías. Su opción completa se denomina “Resumen (n)” cuando existen más categorías de las que se muestran individualmente: en Municipios presenta las nueve categorías principales —manteniendo “Otro” separado siempre que tenga registros— y agrupa las demás en `Municipios restantes (n)`; en Distritos muestra los 13 distritos completos, sin agrupación, con tonos adicionales no repetidos y una leyenda vertical capaz de desplazarse cuando sea necesario; en Lugares conserva los nueve lugares principales y agrupa los demás en `Lugares restantes (n)`; en Causas muestra todas las categorías si hay hasta siete y, cuando hay más, conserva cinco categorías principales y un único segmento neutral `Causas restantes (n)`; en las otras métricas conserva cinco categorías principales y un único segmento neutral de categorías restantes. Así se preserva el total sin presentar decenas de sectores ilegibles ni esconder solo dos causas en un grupo. No se ofrece un Top 5 separado porque duplicaría el resumen de las demás métricas. El control “Resumen (n)” y el segmento de categorías restantes ya comunican este comportamiento; no repetirlo mediante una nota auxiliar debajo de “Mostrar” ni mediante un porcentaje de cobertura, porque el círculo conserva el 100 % del total. La paleta circular institucional combina granate, dorado, azul, verde, terracota, violeta, turquesa, ámbar y malva; el gris queda reservado para las categorías restantes, evitando tonos contiguos difíciles de distinguir. Con hasta seis sectores y nombres breves, la leyenda se sustituye por llamadas exteriores: una línea guía discreta conecta cada sector con el nombre y el valor, porcentaje o ambos elegido en Presentación. No usar puntas de flecha ni duplicar esa información en una leyenda. En pantallas estrechas o con nombres extensos, conservar una leyenda alineada por nombre y métrica; el segmento agrupado se presenta en tono neutral y cursiva. En escritorio, la leyenda vertical se mantiene próxima al círculo sin invadirlo. Cuando se selecciona “Sin etiquetas”, la leyenda queda más angosta: recentrar ópticamente el conjunto pastel–leyenda, separar ambos bloques entre 32 y 40 px y reducir el círculo aproximadamente 5 % para que no parezca adherido a la leyenda. El diámetro exterior y el grosor del anillo permanecen estables y solo se reducen de forma responsiva cuando el ancho o alto disponible realmente lo exige. La exportación de Pastel y Dona recorta el lienzo según los límites reales del círculo, las llamadas o la leyenda, con una composición horizontal compacta y margen uniforme.
- «Otro» es una categoría propia de los registros que no se pudieron asociar a una ubicación del catálogo; «Municipios restantes (n)» agrupa otras categorías municipales y nunca absorbe «Otro» para simplificar la vista circular. No sustituir uno por el otro ni sumar dos veces sus valores. El mismo patrón de nombre se aplica a cualquier otra métrica resumida.
- Con siete o más sectores en Pastel o Dona, evitar una corona de llamadas exteriores que se crucen y no duplicar cifras dentro del círculo. Usar una leyenda compacta junto al gráfico: muestra de color, nombre y métrica elegida forman una sola fila, con el ancho del nombre adaptado al texto más largo y una columna numérica próxima y alineada a la derecha. Mantener todas las categorías visibles, incluidas las pequeñas, con detalle adicional en el tooltip. Con hasta seis sectores y nombres breves, preferir llamadas exteriores discretas en lugar de la leyenda; si el nombre es extenso o la pantalla estrecha, conservar la leyenda. No añadir un selector de estilos de leyenda: la composición se adapta automáticamente a la densidad y al espacio disponible.
- Las etiquetas de datos de Pastel y Dona usan el mismo significado y formato que Columnas y Barras: «Valor» presenta solo la cantidad, «Porcentaje» solo la proporción y «Valor y %» presenta `porcentaje (valor)`, con el valor en un tono secundario. Esta regla se aplica tanto a la leyenda como a las llamadas exteriores; cambiar la disposición por densidad no debe alterar el orden ni la jerarquía de las cifras. «Sin etiquetas» conserva los nombres en la leyenda sin métricas. Cuando se usan llamadas exteriores, reservar margen vertical adicional para sus dos líneas de texto; ni la llamada superior debe rozar el encabezado ni la inferior el pie de la tarjeta. Las vistas con leyenda conservan su diámetro habitual.
- El selector de color de barras y columnas comparte un solo desplegable, pero sus opciones dependen de la cantidad visible: con más de cinco categorías ofrece solo colores sólidos; con dos a cinco, agrupa las paletas cualitativas y los colores sólidos. Por defecto usa color único, incluso al pasar de Top 10 a Top 5: varios colores se aplican únicamente cuando el usuario elige una paleta cualitativa en ese mismo selector. Si luego muestra más de cinco categorías, se usa temporalmente el color sólido preferido y se recupera la paleta cualitativa al volver a dos–cinco. Pastel y Dona conservan su propia paleta al alternar tipos; Mapa y Comparativa conservan igualmente sus colores sin sobrescribir las preferencias de barras. El control cerrado y la marca de selección deben reflejar siempre la opción efectiva, incluso cuando una misma familia de tonos exista como color sólido y como paleta.
- En Dona, el centro queda vacío: el total analizado ya aparece en el encabezado y no se duplica dentro del anillo. Las cantidades y proporciones de cada segmento siguen disponibles en la leyenda o en las llamadas exteriores, según la densidad y la presentación activa.
- La comparativa conserva una vista de columnas agrupadas y muestra como máximo 15 categorías cuando su universo es mayor. El universo completo permanece disponible en “Ver datos”; no intentar comprimir 40 o más pares de columnas dentro del mismo lienzo.
- Municipios ofrece “Mapa” como vista coroplética complementaria. Siempre representa los 43 municipios de Tamaulipas con geometría oficial de INEGI: los municipios sin registros conservan un relleno neutral y no desaparecen. Esta vista no usa Top N; el color expresa intensidad y el tooltip comunica nombre, valor y porcentaje. Los registros agrupados como “Otro” permanecen en el total y en “Ver datos”, pero no generan una nota dentro de la composición geográfica. La intensidad usa rangos adaptativos construidos con los valores positivos disponibles, además de una clase neutral independiente para cero; no usar una escala lineal dominada por un único valor extremo que vuelva indistinguibles los municipios restantes. La leyenda representa esos tonos mediante una barra degradada compacta, con cero y el máximo en sus extremos, no mediante una sucesión de cuadros. El territorio es el único foco de esta presentación: permanece ampliado y centrado en el lienzo, con espacio óptico equilibrado a ambos lados. Por su silueta alta y estrecha, en escritorio el lienzo del mapa dispone de mayor altura que las gráficas ordinarias para aumentar su escala sin deformarlo. Al exportar PNG o PDF, recortar automáticamente el espacio vacío que rodea esa composición y conservar un margen corto uniforme; la descarga debe resultar vertical y ajustada al territorio, no heredar el ancho panorámico de la tarjeta. No añadir títulos territoriales, tarjetas, divisores ni rankings laterales; el orden de municipios ya pertenece a la vista de Columnas y repetirlo aquí reduce el espacio útil y fragmenta la lectura. El hover no duplica el nombre del municipio sobre el polígono cuando ya existe un tooltip.
- Comparativa ofrece “Mapa de calor” para cruces matriciales. Las columnas representan la segunda dimensión y las filas la primera; cada celda conserva el conteo real y el porcentaje sobre el total analizado. “Distrito de residencia vs. distrito de defunción” cruza explícitamente ambos campos y exige los dos como datos completos. La vista reutiliza el mismo límite Top que la comparativa de columnas cuando la segunda dimensión es extensa.
- “Lugar de defunción por municipio” pertenece a Comparativa: coloca los municipios de defunción en columnas y los lugares de defunción en filas; en Columnas representa los lugares como segmentos apilados. Exige ambos campos como datos completos, oculta el filtro de municipio porque duplicaría una dimensión y conserva el Top de municipios para evitar comprimir los 44 dentro del lienzo.
- Mapa y mapa de calor son presentaciones de la respuesta ya filtrada, no consultas paralelas. Fechas, causa, sexo, edad y los demás filtros compatibles se aplican una sola vez en el alcance estadístico compartido; cambiar entre columnas, mapa o mapa de calor no modifica el universo ni reinterpreta los filtros.
- La escala también responde a la densidad real de la métrica: una distribución de dos categorías usa un diámetro contenido, tres o cuatro usan un tamaño intermedio y cinco o más el tamaño estándar. Esta adaptación puede variar entre métricas, pero no al cambiar el Top dentro de la misma densidad. En paletas secuenciales, repartir los tonos disponibles cuando existan cinco categorías o menos para evitar que sectores contiguos resulten casi indistinguibles. Las barras con dos a cuatro categorías también limitan su grosor y centran el área de trazado mediante márgenes laterales adaptativos; no extender dos columnas a lo largo de todo el lienzo ni reducir la tarjeta solo para llenar el espacio.
...
- La visualización conserva título, total y estados de carga, vacío y error. Cuando el total analizado sea cero, no inicializar una gráfica con ejes o líneas vacías: ocultar el lienzo y mostrar dentro de su misma área un estado neutral con el icono estadístico del sistema, el título `No hay datos para los filtros seleccionados.`, una explicación breve y la acción secundaria `Limpiar filtros` cuando existan criterios aplicados. Mantener visibles el periodo, el total `0` y los chips que explican la consulta; deshabilitar Ver datos y las descargas, y ocultar el origen. Si existen registros excluidos pero ninguno completo para la métrica, cambiar el mensaje para explicarlo y conservar la acción `Revisar` del aviso de calidad. El estado vacío usa la misma tipografía, superficie blanca, colores neutrales, borde y foco de los demás estados del sistema; no introducir ilustraciones, tarjetas flotantes ni colores de éxito. Su altura es acotada y adaptativa: parte de una altura útil mínima y, en escritorio, puede crecer para acompañar un panel de Presentación más largo. La altura del lienzo interno se calcula antes de inicializar la gráfica y debe ocupar el cuerpo completo de la tarjeta; el eje, la leyenda y la tarjeta terminan juntos, sin una banda inferior vacía. En móvil, los grupos de presentación se apilan y el popover de filtros se presenta como panel inferior con acciones de al menos 40 px.
- Los estados del lienzo conservan la identidad semántica compartida, pero no heredan la geometría de una fila de tabla. Las acciones `Limpiar filtros`, `Revisar` y `Reintentar` reutilizan exactamente el mismo botón secundario delineado: altura, relleno, borde, radio, tipografía, hover y foco son comunes; la diferencia entre estados se comunica únicamente mediante el icono y el texto. Cuando la carga inicial de una gráfica falla, el propio lienzo muestra un bloque neutral, compacto y centrado, sin tarjeta ni franja de color superpuesta; el rojo semántico se reserva para el icono de advertencia. El título, la explicación y la única acción secundaria “Reintentar” mantienen la jerarquía tipográfica, los controles y los estados de foco del sistema. Una franja roja inline se reserva para fallos de actualización cuando la gráfica anterior permanece visible y se necesita advertir que sus datos pueden estar desactualizados.
- La tarjeta incluye un pie compacto de trazabilidad con el origen de los registros filtrados. El resumen prioriza el número de importaciones y menciona la captura manual únicamente cuando aporta registros. “Ver detalle del origen” abre un popover anclado al pie y cambia a “Ocultar detalle del origen” mientras permanece abierto; su hover subraya solo el texto de la acción, sin aplicar fondo a toda la fila, y conserva estados presionado y foco, una entrada breve y cierre con Escape o clic exterior. El detalle muestra el nombre original y la cantidad aportada por cada carga, además de los registros manuales; nunca presenta el archivo como descargable si el sistema solo conserva sus metadatos. En pantallas estrechas, la cantidad se coloca debajo de los metadatos del archivo para no comprimir su nombre. En “Ver datos”, no repetir el origen como columna en cada fila: el bloque contextual de la gráfica ofrece un selector `Origen` con `Todos los orígenes` como opción inicial, cada archivo y `Captura manual`, siempre acompañados por su cantidad. La selección filtra la tabla y la descarga CSV, conserva visible el total original utilizado por la gráfica y muestra los nombres completos dentro de la lista.
- El origen se calcula sobre todo el universo que cumple los filtros de datos, antes de aplicar Top N. No contar filas fallidas, duplicadas, omitidas ni importaciones revertidas. La suma del detalle debe coincidir con el total filtrado, aunque el total visible de la gráfica represente solo una selección de categorías.
